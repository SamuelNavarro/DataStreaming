...
redisStreamingDF = redisRawStreamingDF.selectExpr("cast(value as string)")
...
customerRiskStreamingDF = emailAndBirthYearStreamingDF.join(customerRiskStreamingDF,
                                                            expr("""
                                                                 email=customer
                                                                 """))
...
customerRiskStreamingDF.selectExpr("CAST(customer AS STRING)", "to_json(struct(*)) AS value")    \
        .writeStream                                                                                    \
        .format("kafka")                                                                                \
        .option("kafka.bootstrap.servers","kafka:19092")                                                \
...
################### Extra ######################
#########################################
# With this we can enable both kafka/console consumer.
"""
